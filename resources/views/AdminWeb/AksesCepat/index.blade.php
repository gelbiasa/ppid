  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $detailAksesCepatUrl, 'create')
                )
                <button onclick="modalAction('{{ url($detailAksesCepatUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari judul Informasi Akses Cepat" 
                     value="{{ $search ?? '' }}">
        ...
        <div class="table-responsive" id="table-container">
          @include('AdminWeb.AksesCepat.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  // URL dinamis untuk Management detailAksesCepat
  var detailAksesCepatUrl = '{{ $detailAksesCepatUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadAksesCepatData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadAksesCepatData(page, search);
  });

  // Fungsi untuk memuat data detailAksesCepat
  function loadAksesCepatData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + detailAksesCepatUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadAksesCepatData(currentPage, search);
  }
