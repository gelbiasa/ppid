...
@section('content')
  <div class="card card-outline card-primary">
    ...
    <div class="row align-items-center">
      ...
      SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriMediaUrl, 'create')
    )
          <button onclick="modalAction('{{ url($kategoriMediaUrl . '/addData') }}')" class="btn btn-sm btn-success">
    ...
    <div class="row mb-3">
      ...
      <form id="searchForm" class="d-flex">
        <input type="text" name="search" class="form-control" placeholder="Cari kategori media"
        value="{{ $search ?? '' }}">
  ...
  @endif
    ...
    <div class="table-responsive" id="table-container">
      @include('AdminWeb.MediaDinamis.data')
    </div>
    </div>
  </div>
...
@push('js')
  <script>
    ...
    // URL dinamis untuk Management KategoriMedia
    var kategoriMediaUrl = '{{ $kategoriMediaUrl }}';

    // Handle search form submission
    $('#searchForm').on('submit', function (e) {
      e.preventDefault();
      var search = $(this).find('input[name="search"]').val();
      loadKategoriMediaData(1, search);
    });
    ...
    $(document).on('click', '.pagination a', function (e) {
      ...
      loadKategoriMediaData(page, search);
    });

    // Fungsi untuk memuat data KategoriMedia
    function loadKategoriMediaData(page, search) {
      $.ajax({
      url: '{{ url('') }}/' + kategoriMediaUrl + '/getData',
      type: 'GET',
      data: {
        page: page,
        search: search
    ...
    function reloadTable() {
      ...
      var search = $('#searchForm input[name="search"]').val();
      loadKategoriMediaData(currentPage, search);
    }
