  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $detailPintasanLainnyaUrl, 'create')
                )
                <button onclick="modalAction('{{ url($detailPintasanLainnyaUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari judul detail pintasan" 
                     value="{{ $search ?? '' }}">
        ...
        <div class="table-responsive" id="table-container">
          @include('AdminWeb/DetailPintasanLainnya.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  var detailPintasanLainnyaUrl = '{{ $detailPintasanLainnyaUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadDetailPintasanLainnyaData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadDetailPintasanLainnyaData(page, search);
  });

  // Fungsi untuk memuat data KategoriForm
  function loadDetailPintasanLainnyaData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + detailPintasanLainnyaUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadDetailPintasanLainnyaData(currentPage, search);
  }
