          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriTahunLHKPNUrl, 'create')
                )
                <button onclick="modalAction('{{ url($kategoriTahunLHKPNUrl . '/addData') }}')" class="btn btn-sm btn-success">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari tahun atau judul LHKPN" 
                     value="{{ $search ?? '' }}">
...
<script>
  ...
  // URL dinamis untuk Management kategoriTahunLHKPN
  var kategoriTahunLHKPNUrl = '{{ $kategoriTahunLHKPNUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadKategoriTahunLHKPNData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadKategoriTahunLHKPNData(page, search);
  });

  // Fungsi untuk memuat data kategoriTahunLHKPN
  function loadKategoriTahunLHKPNData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + kategoriTahunLHKPNUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadKategoriTahunLHKPNData(currentPage, search);
  }
