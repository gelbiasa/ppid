         ...
         <tr>
             ...
             <td>
                @if(
                    ...
                    SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriRegulasiUrl, 'update')
                )
                    <button class="btn btn-sm btn-warning"
                        onclick="modalAction('{{ url($kategoriRegulasiUrl . '/editData/' . $item->kategori_reg_id) }}')">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                @endif
                <button class="btn btn-sm btn-info"
                    onclick="modalAction('{{ url($kategoriRegulasiUrl . '/detailData/' . $item->kategori_reg_id) }}')">
                    <i class="fas fa-eye"></i> Detail
                </button>
                @if(
                    Auth::user()->level->hak_akses_kode === 'SAR' ||
                    SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriRegulasiUrl, 'delete')
                )
                    <button class="btn btn-sm btn-danger"
                        onclick="modalAction('{{ url($kategoriRegulasiUrl . '/deleteData/' . $item->kategori_reg_id) }}')">
