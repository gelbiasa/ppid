  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriBeritaUrl, 'create')
                )
                <button onclick="modalAction('{{ url($kategoriBeritaUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
        ...
        <div class="table-responsive" id="table-container">
          @include('AdminWeb.BeritaDinamis.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  // URL dinamis untuk Management KategoriBerita
  var kategoriBeritaUrl = '{{ $kategoriBeritaUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadBeritaDinamisData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadBeritaDinamisData(page, search);
  });

  // Fungsi untuk memuat data KategoriForm
  function loadBeritaDinamisData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + kategoriBeritaUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadBeritaDinamisData(currentPage, search);
  }
