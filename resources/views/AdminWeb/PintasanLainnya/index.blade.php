  ...
  <div class="card card-outline card-primary">
          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $kategoriPintasanLainnyaUrl, 'create')
                )
                <button onclick="modalAction('{{ url($kategoriPintasanLainnyaUrl . '/addData') }}')" class="btn btn-sm btn-success">
      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari nama pintasan lainnya atau kategori" 
                     value="{{ $search ?? '' }}">
        ...
        <div class="table-responsive" id="table-container">
          @include('AdminWeb.PintasanLainnya.data')
        </div>
      </div>
  </div>
...
<script>
  ...
  // URL dinamis untuk Management KategoriPintasanLainnya
  var kategoriPintasanLainnyaUrl = '{{ $kategoriPintasanLainnyaUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadkategoriPintasanLainnyaData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadkategoriPintasanLainnyaData(page, search);
  });

  // Fungsi untuk memuat data kategoriPintasanLainnya
  function loadkategoriPintasanLainnyaData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + kategoriPintasanLainnyaUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
  ...
  function reloadTable() {
    ...
    var search = $('#searchForm input[name="search"]').val();
    loadkategoriPintasanLainnyaData(currentPage, search);
  }
