<div class="modal-header">
 ...
 <form id="form-update-detail-media" enctype="multipart/form-data">
     <div class="modal-body">
         <div class="form-group">
             ...
             <select class="form-control" id="fk_m_media_dinamis" name="t_detail_media_dinamis[fk_m_media_dinamis]" required>
                 ...
                 @foreach($kategoris as $kategori)
                     <option value="{{ $kategori->media_dinamis_id }}" {{ $detailMediaDinamis->fk_m_media_dinamis == $kategori->media_dinamis_id ? 'selected' : '' }}>
         ...
         <div id="media-file-input" class="form-group" style="display: {{ $detailMediaDinamis->dm_type_media == 'file' ? 'block' : 'none' }};">
             ...
             <div class="custom-file">
                 ...
                 <label class="custom-file-label" for="media_file">Pilih file baru (opsional)</label>
             </div>
             <small class="form-text text-muted">Format yang didukung: JPG, JPEG, PNG, GIF, PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX.</small>
             ...
             @if($detailMediaDinamis->dm_type_media == 'file')
                 <div class="mt-2">
                     ...
                     @if($isImage)
                         <img src="{{ asset($detailMediaDinamis->dm_media_upload) }}" class="img-thumbnail" style="max-height: 150px;">
                     @else
                         <a href="{{ asset($detailMediaDinamis->dm_media_upload) }}" target="_blank" class="btn btn-sm btn-info">
                             <i class="fas fa-file"></i> {{ basename($detailMediaDinamis->dm_media_upload) }}
                         </a>
                     @endif
                 </div>
                 <input type="hidden" name="t_detail_media_dinamis[dm_media_upload]" value="{{ $detailMediaDinamis->dm_media_upload }}">
             @endif
         ...
         <div id="media-link-input" class="form-group" style="display: {{ $detailMediaDinamis->dm_type_media == 'link' ? 'block' : 'none' }};">
             <label for="dm_media_upload_link">Link Media <span class="text-danger">*</span></label>
             <input type="text" class="form-control" id="dm_media_upload_link" name="t_detail_media_dinamis[dm_media_upload]" 
                    placeholder="https://example.com/media" value="{{ $detailMediaDinamis->dm_type_media == 'link' ? $detailMediaDinamis->dm_media_upload : '' }}">
