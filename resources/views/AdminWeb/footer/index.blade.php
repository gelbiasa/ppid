          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $detailFooterUrl, 'create')
                )
                <button onclick="modalAction('{{ url($detailFooterUrl . '/addData') }}')" class="btn btn-sm btn-success">
          ...
          <div class="col-md-6">
            <form id="searchForm" class="d-flex">
              <input type="text" name="search" class="form-control" 
                     placeholder="Cari judul atau kategori footer" 
                     value="{{ $search ?? '' }}">
...
<script>
  ...
  var detailFooterUrl = '{{ $detailFooterUrl }}';
  ...
  function loadFooterData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + detailFooterUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
