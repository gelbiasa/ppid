...
@section('content')
  <div class="card card-outline card-primary">
    ...
    <div class="row align-items-center">
      ...
      <div class="card-tools">
        ...
        @if(
          ...
          SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $managementLevelUrl, 'create')
        )
          <button onclick="modalAction('{{ url($managementLevelUrl . '/addData') }}')" class="btn btn-sm btn-success">
    ...
    <div class="row mb-3">
      ...
      <form id="searchForm" class="d-flex">
        <input type="text" name="search" class="form-control" placeholder="Cari kode atau nama level"
        value="{{ $search ?? '' }}">
  ...
  @endif
    ...
    <div class="table-responsive" id="table-container">
      @include('ManagePengguna/ManageLevel/data')
    </div>
    </div>
  </div>
...
@push('js')
  <script>
    ...
    // URL dinamis untuk Management Level
    var managementLevelUrl = '{{ $managementLevelUrl }}';

    // Handle search form submission
    $('#searchForm').on('submit', function (e) {
      e.preventDefault();
      var search = $(this).find('input[name="search"]').val();
      loadLevelData(1, search);
    });
    ...
    $(document).on('click', '.pagination a', function (e) {
      ...
      loadLevelData(page, search);
    });

    // Fungsi untuk memuat data level
    function loadLevelData(page, search) {
      $.ajax({
      url: '{{ url('') }}/' + managementLevelUrl + '/getData',
      type: 'GET',
      data: {
        page: page,
        search: search
    ...
    function reloadTable() {
      ...
      var search = $('#searchForm input[name="search"]').val();
      loadLevelData(currentPage, search);
    }
