          ...
          <div class="col-md-6 text-right">
            <div class="card-tools">
              ...
              @if(
                ...
                SetHakAksesModel::cekHakAkses(Auth::user()->user_id, $timelineUrl, 'create')
              )
                <button onclick="modalAction('{{ url($timelineUrl . '/addData') }}')" class="btn btn-sm btn-success">
...
<script>
  ...
  var timelineUrl = '{{ $timelineUrl }}';

  // Handle search form submission
  $('#searchForm').on('submit', function (e) {
    e.preventDefault();
    var search = $(this).find('input[name="search"]').val();
    loadTimelineData(1, search);
  });
  ...
  $(document).on('click', '.pagination a', function (e) {
    ...
    loadTimelineData(page, search);
  });

  function loadTimelineData(page, search) {
    $.ajax({
    url: '{{ url('') }}/' + timelineUrl + '/getData',
    type: 'GET',
    data: {
      page: page,
      search: search
